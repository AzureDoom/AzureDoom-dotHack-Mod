...
import mod.azure.dothack.entity.ai.goal.RangedMagicAttackGoal;
...
public class AtoliEntity extends NPCEntity implements IRangedAttackMob {

	private final RangedMagicAttackGoal<AtoliEntity> aiArrowAttack = new RangedMagicAttackGoal<>(this, 1.0D, 20, 15.0F);
	private final MeleeAttackGoal aiAttackOnCollide = new MeleeAttackGoal(this, 1.2D, false) {
		public void resetTask() {
			super.resetTask();
			AtoliEntity.this.setAggroed(false);
		}

		public void startExecuting() {
			super.startExecuting();
			AtoliEntity.this.setAggroed(true);
		}
	};

	public AtoliEntity(EntityType<AtoliEntity> entityType, World worldIn) {
		super(entityType, worldIn);
		this.setItemStackToSlot(EquipmentSlotType.MAINHAND, new ItemStack(DotHackItems.HARVESTCLERIC1.get()));
		this.setCombatTask();
	}
	...
	public static boolean spawning(EntityType<AtoliEntity> p_223337_0_, IWorld p_223337_1_, SpawnReason reason,
	...
	public void setCombatTask() {
		if (this.world != null && !this.world.isRemote) {
			...
			ItemStack itemstack = this.getHeldItem(ProjectileHelper.getHandWith(this, DotHackItems.WAVEMASTER43.get()));
			if (itemstack.getItem() instanceof MagicItem) {
				int i = 20;
	...
	public void attackEntityWithRangedAttack(LivingEntity target, float distanceFactor) {
		ItemStack itemstack = this
				.findAmmo(this.getHeldItem(ProjectileHelper.getHandWith(this, DotHackItems.WAVEMASTER43.get())));
		ManaEntity abstractarrowentity = this.fireArrowa(itemstack, distanceFactor);
		if (this.getHeldItemMainhand().getItem() instanceof MagicItem)
			abstractarrowentity = ((MagicItem) this.getHeldItemMainhand().getItem()).customeArrow(abstractarrowentity);
	...
	protected ManaEntity fireArrowa(ItemStack arrowStack, float distanceFactor) {
		return AtoliEntity.fireArrow(this, arrowStack, distanceFactor);
	}

	public static ManaEntity fireArrow(LivingEntity shooter, ItemStack arrowStack, float distanceFactor) {
		ManaItem arrowitem = (ManaItem) (arrowStack.getItem() instanceof ManaItem ? arrowStack.getItem()
				: DotHackItems.MANA.get());
		ManaEntity abstractarrowentity = arrowitem.createArrow(shooter.world, arrowStack, shooter);
